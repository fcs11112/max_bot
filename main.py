...
def has_aspose_credentials() -> bool:
    return bool(os.getenv("ASPOSE_CLIENT_ID") and os.getenv("ASPOSE_CLIENT_SECRET"))


def configured_pdf_converters() -> list[str]:
    converters = []
    if os.getenv("CLOUDCONVERT_API_KEY"):
        converters.append("CloudConvert")
    if has_aspose_credentials():
        converters.append("Aspose")
    if os.getenv("CONVERTAPI_SECRET"):
        converters.append("ConvertAPI")
    if os.getenv("ALLOW_LIBREOFFICE_FALLBACK", "false").lower() == "true":
        converters.append("LibreOffice fallback")
    return converters


def aspose_api_base() -> str:
    return os.getenv("ASPOSE_API_BASE", "https://api.aspose.cloud").rstrip("/")


def get_aspose_access_token() -> str:
    global ASPOSE_TOKEN, ASPOSE_TOKEN_EXPIRES_AT

    if ASPOSE_TOKEN and time.time() < ASPOSE_TOKEN_EXPIRES_AT:
        return ASPOSE_TOKEN

    client_id = os.getenv("ASPOSE_CLIENT_ID", "")
    client_secret = os.getenv("ASPOSE_CLIENT_SECRET", "")
    if not client_id or not client_secret:
        raise RuntimeError("ASPOSE_CLIENT_ID и ASPOSE_CLIENT_SECRET не заданы в переменных сервера")

    token_urls = [
        f"{aspose_api_base()}/connect/token",
        f"{aspose_api_base()}/oauth2/token",
    ]
    errors = []
    ...
    for token_url in token_urls:
        ...
        if not response.ok:
            errors.append(f"{token_url}: HTTP {response.status_code}: {response.text}")
            continue

        data = response.json()
        access_token = data.get("access_token")
        if not access_token:
            errors.append(f"{token_url}: access_token не найден в ответе: {data}")
            continue

        ASPOSE_TOKEN = access_token
        ASPOSE_TOKEN_EXPIRES_AT = time.time() + int(data.get("expires_in", 3600)) - 60
        return ASPOSE_TOKEN

    raise RuntimeError("Aspose не выдал access token:\n" + "\n".join(errors))
...
def convert_pptx_to_pdf_with_aspose(pptx_path: Path, output_dir: Path) -> Path:
    token = get_aspose_access_token()
    pdf_path = output_dir / f"{pptx_path.stem}.pdf"
    url = f"{aspose_api_base()}/v3.0/slides/convert/Pdf"
    ...
    if not response.ok:
        raise RuntimeError(f"Aspose HTTP {response.status_code}:\n{response.text}")

    pdf_path.write_bytes(response.content)
    if not pdf_path.exists() or pdf_path.stat().st_size == 0:
        raise RuntimeError("Aspose вернул пустой PDF.")

    return pdf_path


def convert_pptx_to_pdf_with_convertapi(pptx_path: Path, output_dir: Path) -> Path:
    secret = os.getenv("CONVERTAPI_SECRET", "")
    if not secret:
        raise RuntimeError("CONVERTAPI_SECRET не задан в Railway Variables")
    ...
    with pptx_path.open("rb") as file:
        response = requests.post(
            url,
            params={"Secret": secret, "StoreFile": "false", "EmbedFonts": "true"},
            files={"File": (pptx_path.name, file, "application/vnd.openxmlformats-officedocument.presentationml.presentation")},
            timeout=180,
        )

    if not response.ok:
        raise RuntimeError(f"ConvertAPI HTTP {response.status_code}:\n{response.text}")
    ...
    if "application/json" in content_type:
        ...
        file_url = None
        if files:
            file_url = files[0].get("Url") or files[0].get("url")
        if not file_url:
            raise RuntimeError(f"ConvertAPI не вернул PDF-файл:\n{json.dumps(data, ensure_ascii=False, indent=2)}")
    ...
    if not pdf_path.exists() or pdf_path.stat().st_size == 0:
        raise RuntimeError("ConvertAPI вернул пустой PDF.")
...
def convert_pptx_to_pdf(pptx_path: Path, output_dir: Path) -> Path:
    errors = []
    preferred_converter = os.getenv("PDF_CONVERTER", "auto").strip().lower()

    if preferred_converter in {"cloudconvert", "cloud"}:
        print("PDF converter forced: CloudConvert")
        return convert_pptx_to_pdf_with_cloudconvert(pptx_path, output_dir)

    if preferred_converter in {"aspose", "asposecloud"}:
        print("PDF converter forced: Aspose")
        return convert_pptx_to_pdf_with_aspose(pptx_path, output_dir)

    if preferred_converter in {"convertapi", "convert"}:
        print("PDF converter forced: ConvertAPI")
        return convert_pptx_to_pdf_with_convertapi(pptx_path, output_dir)

    if preferred_converter == "libreoffice":
        print("PDF converter forced: LibreOffice")
        return convert_pptx_to_pdf_with_libreoffice(pptx_path, output_dir)

    if preferred_converter not in {"", "auto"}:
        raise RuntimeError("PDF_CONVERTER должен быть auto, cloudconvert, aspose, convertapi или libreoffice")

    if os.getenv("CLOUDCONVERT_API_KEY"):
        try:
            print("PDF converter: CloudConvert")
            return convert_pptx_to_pdf_with_cloudconvert(pptx_path, output_dir)
        except Exception as exc:
            errors.append(f"CloudConvert: {exc}")
            print(f"CloudConvert failed: {exc}")

    if has_aspose_credentials():
        try:
            print("PDF converter: Aspose")
            return convert_pptx_to_pdf_with_aspose(pptx_path, output_dir)
        except Exception as exc:
            errors.append(f"Aspose: {exc}")
            print(f"Aspose failed: {exc}")

    if os.getenv("CONVERTAPI_SECRET"):
        try:
            print("PDF converter: ConvertAPI")
            return convert_pptx_to_pdf_with_convertapi(pptx_path, output_dir)
        except Exception as exc:
            errors.append(f"ConvertAPI: {exc}")
            print(f"ConvertAPI failed: {exc}")

    if os.getenv("ALLOW_LIBREOFFICE_FALLBACK", "false").lower() == "true":
        try:
            print("PDF converter: LibreOffice")
            return convert_pptx_to_pdf_with_libreoffice(pptx_path, output_dir)
        except Exception as exc:
            errors.append(f"LibreOffice: {exc}")

    raise RuntimeError(
        "Не удалось сконвертировать PPTX в PDF через внешние конвертеры. "
        "LibreOffice fallback отключён, чтобы не портить макет.\n\n"
        + "\n\n".join(errors)
    )
...
def read_pptx_contents(pptx_path: Path) -> dict[str, bytes]:
    with zipfile.ZipFile(pptx_path, "r") as zin:
        return {name: zin.read(name) for name in zin.namelist()}


def remove_slide_parts(contents: dict[str, bytes]) -> dict[str, bytes]:
    return {
        name: data
...
def write_pptx_from_contents(contents: dict[str, bytes], output_path: Path) -> None:
    with zipfile.ZipFile(output_path, "w", zipfile.ZIP_DEFLATED) as zout:
        for name, data in contents.items():
            zout.writestr(name, data)


def build_single_pptx_files(
    template_path: Path,
    df: pd.DataFrame,
    case_code: str,
    output_dir: Path,
) -> list[tuple[str, Path]]:
    template_contents = read_pptx_contents(template_path)
    slide_files = sorted(
        [name for name in template_contents if re.fullmatch(r"ppt/slides/slide\d+\.xml", name)],
        key=lambda name: int(re.search(r"slide(\d+)\.xml", name).group(1)),
    )
    ...
    presentation_rels_key = "ppt/_rels/presentation.xml.rels"
    original_presentation_xml = template_contents["ppt/presentation.xml"].decode("utf-8")
    original_presentation_rels_xml = template_contents[presentation_rels_key].decode("utf-8")
    original_content_types_xml = template_contents["[Content_Types].xml"].decode("utf-8")

    result = []
    for row_index, row in df.iterrows():
        replacements = build_replacements(row, df.columns, case_code)
        name = sanitize_filename(row[df.columns[0]], fallback=f"presentation_{row_index + 1}")
        contents = remove_slide_parts(template_contents)

        slide_ids = []
        slide_relationships = []
        start_rel_num = next_relationship_start(original_presentation_rels_xml)

        for index, slide_file in enumerate(slide_files, start=1):
            rel_id = f"rId{start_rel_num + index - 1}"
            contents[f"ppt/slides/slide{index}.xml"] = replace_in_xml_text(template_contents[slide_file], replacements)

            rels_key = slide_file.replace("ppt/slides/", "ppt/slides/_rels/") + ".rels"
            if template_contents.get(rels_key):
                contents[f"ppt/slides/_rels/slide{index}.xml.rels"] = template_contents[rels_key]
        ...
        presentation_xml = re.sub(
            r"<p:sldIdLst>.*?</p:sldIdLst>",
            f"<p:sldIdLst>{''.join(slide_ids)}</p:sldIdLst>",
            original_presentation_xml,
            flags=re.DOTALL,
        )
        contents["ppt/presentation.xml"] = presentation_xml.encode("utf-8")

        presentation_rels_xml = re.sub(
            r'<Relationship[^>]+Type="http://schemas\.openxmlformats\.org/officeDocument/2006/relationships/slide"[^>]*/>',
            "",
            original_presentation_rels_xml,
        )
        presentation_rels_xml = presentation_rels_xml.replace("</Relationships>", f"{''.join(slide_relationships)}</Relationships>")
        contents[presentation_rels_key] = presentation_rels_xml.encode("utf-8")
        ...
        pptx_path = unique_path(output_dir, f"{name}.pptx")
        write_pptx_from_contents(contents, pptx_path)
        result.append((name, pptx_path))
...
def generate_pdf_zip(template_path: Path, excel_path: Path, user_id: str, case_code: str) -> str:
    ...
    if os.getenv("CLOUDCONVERT_API_KEY") or has_aspose_credentials() or os.getenv("CONVERTAPI_SECRET"):
        ...
        single_pptx_files = build_single_pptx_files(template_path, df, case_code, user_output_dir)
        ...
        with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zipf:
            for _, pptx_path in single_pptx_files:
                zipf.write(pptx_path, arcname=f"PPTX/{pptx_path.name}")
            ...
            for index, name in enumerate(names):
                ...
                with page_path.open("wb") as file:
                    writer.write(file)
                zipf.write(page_path, arcname=f"PDF/{page_path.name}")
    ...
    for index, row in df.iterrows():
        ...
        generated_pptx_files.append(pptx_path)
    ...
    with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zipf:
        for file in generated_pptx_files:
            zipf.write(file, arcname=f"PPTX/{file.name}")
        for file in generated_pdfs:
            zipf.write(file, arcname=f"PDF/{file.name}")
...
def debug():
    ...
    return (
        ...
        f"ASPOSE_CLIENT_ID: {'задан' if os.getenv('ASPOSE_CLIENT_ID') else 'не задан'}\n"
        f"ASPOSE_CLIENT_SECRET: {'задан' if os.getenv('ASPOSE_CLIENT_SECRET') else 'не задан'}\n"
        f"CONVERTAPI_SECRET: {'задан' if os.getenv('CONVERTAPI_SECRET') else 'не задан'}\n"
        f"PDF_CONVERTER: {os.getenv('PDF_CONVERTER', 'auto')}\n"
        f"ALLOW_LIBREOFFICE_FALLBACK: {os.getenv('ALLOW_LIBREOFFICE_FALLBACK', 'false')}\n"
        f"PDF converter: {' -> '.join(configured_pdf_converters()) or 'LibreOffice local'} batch\n"
